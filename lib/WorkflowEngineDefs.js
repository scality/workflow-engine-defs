class WorkflowEngineDefs {
  ...
  get functionSubTypes() {
    return [this.SUB_TYPE_FISSION, this.SUB_TYPE_AWS_LAMBDA, this.SUB_TYPE_AZURE_FUNCTION, this.SUB_TYPE_GOOGLE_CLOUD_FUNCTION];
  }
  ...
  findNodes(type) {
    const nodes = []; // eslint-disable-next-line no-restricted-syntax
  ...
  findNextNodes(refNode, name) {
    const nextNodes = []; // eslint-disable-next-line no-restricted-syntax
  ...
  findPrevNodes(refNode, name) {
    const prevNodes = []; // eslint-disable-next-line no-restricted-syntax
  ...
  _isCyclicInternal(node, visited, nodeStack) {
    ...
    if (visited[node.id] === undefined) {
      ...
      visited[node.id] = true; // eslint-disable-next-line no-param-reassign

      nodeStack[node.id] = true; // iterate over adjacent nodes
      // eslint-disable-next-line no-restricted-syntax
      ...
      for (const port of node.ports) {
        ...
        for (const linkId of port.links) {
          ...
          if (link) {
            ...
            if (_node) {
              if (!(visited[_node.id] === true) && this._isCyclicInternal(_node, visited, nodeStack)) {
                return true;
  ...
  _isCyclic() {
    const visited = {};
    const nodeStack = {}; // eslint-disable-next-line no-restricted-syntax
  ...
  checkModel() {
    let foundSource = false; // check nodes
    // eslint-disable-next-line no-restricted-syntax
    ...
    for (const node of this.model.nodes) {
      ...
      if (node.type === this.TYPE_DATA) {
        if (foundSource) {
          return {
            isValid: false,
            message: `"${node.name}" only one source per workflow`
          };
        }
        ...
        if (node.subType === this.SUB_TYPE_ADVANCED) {
          if (node.script === undefined) {
            return {
              isValid: false,
              message: `"${node.name}" data missing filter script`
            };
          }
          ...
          if (node.key === undefined) {
            return {
              isValid: false,
              message: `"${node.name}" data missing bucket`
            };
          ...
          return {
            isValid: false,
            message: `"${node.name}": ${node.subType} not supported`
          };
        }
        ...
        if (node.subType === this.SUB_TYPE_ADVANCED) {
          ...
          if (node.key === undefined) {
            return {
              isValid: false,
              message: `"${node.name}" search missing key`
            };
          }
          ...
          if (node.script === undefined) {
            return {
              isValid: false,
              message: `"${node.name}" search missing filter script`
            };
          }
        } else if (node.subType === this.SUB_TYPE_BASIC) {
          if (node.key === undefined) {
        ...
        if (node.cronRule === undefined) {
          return {
            isValid: false,
            message: `"${node.name}" shall define cron rule`
          };
        }
        ...
        if (node.subType === this.SUB_TYPE_FISSION) {
          if (node.func === undefined) {
            return {
              isValid: false,
              message: `"${node.name}" func must be defined`
            };
          }
          ...
          if (node.asynchronous === undefined) {
            return {
              isValid: false,
              message: `"${node.name}" asynchronous not defined`
            };
          }
          ...
          if (node.endpoint === undefined) {
            return {
              isValid: false,
              message: `"${node.name}" endpoint must be defined`
            };
          }
          ...
          if (node.postData === undefined) {
            return {
              isValid: false,
              message: `"${node.name}" postData not defined`
            };
          }
        } else {
          return {
      ...
      } else if (node.type === this.TYPE_STOPPER) {// OK
      } else if (node.type === this.TYPE_UPDATE) {// OK
      } else {
        return {
          isValid: false,
      ...
      for (const port of node.ports) {
        ...
        for (const linkId of port.links) {
          ...
          if (this.findLink(linkId) === null) {
            throw new Error(`"${node.name}" ${linkId} is not a valid link`);
          }
        }
      }
    } // check links
    ...
    for (const link of this.model.links) {
      ...
      if (node) {
        const sourcePort = this.findNodePort(node, link.sourcePort); // check that sourcePort is a port of the source node
        // eslint-disable-next-line

        if (sourcePort != null) {
          // check port name
          if (sourcePort.name !== 'output' && sourcePort.name !== 'output2') {
            return {
              isValid: false,
      ...
      if (node) {
        const targetPort = this.findNodePort(node, link.targetPort); // check that targetPort is a port of the target node
        // eslint-disable-next-line
